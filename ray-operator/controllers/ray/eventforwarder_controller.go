...
// EventForwarderReconciler implements the Selective Event Forwarder: it
// watches Kubernetes Events involving Nodes and re-emits them onto the Ray
// custom resources (RayCluster, and the owning RayJob if any) whose Pods are
// scheduled on those Nodes, so they surface in the Ray Dashboard's Platform
// Events tab.
//
...
func NewEventForwarderReconciler(mgr manager.Manager, options EventForwarderOptions) *EventForwarderReconciler {
	return &EventForwarderReconciler{
		...
		Recorder:  mgr.GetEventRecorderFor("kuberay-event-forwarder"),
...
func (r *EventForwarderReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	for target := range targets {
		...
		r.Recorder.AnnotatedEventf(obj,
			map[string]string{
				"ray.io/source-node":       nodeName,
				"ray.io/source-event-uid":  string(src.UID),
				"ray.io/source-event-name": src.Name,
			},
			src.Type, forwardedEventReason,
			"Infrastructure failure detected on Node %q (reason: %s, source: %s): %s",
			nodeName, src.Reason, eventSource(src), src.Message)
		logger.Info("forwarded node event to Ray resource",
